...
const USER_AGENT = "SoundTouchBridge/0.1.0";
const API_HOSTS = [
...
function buildSearchPath(criteria) {
...
async function mapCompatibleStations(stations) {
  const compatible = [];
  const candidates = stations.filter(isStationMetadataCompatible).slice(0, SEARCH_LIMIT);
  let nextIndex = 0;

  async function worker() {
    while (nextIndex < candidates.length && compatible.length < RESULT_LIMIT) {
      const currentIndex = nextIndex;
      nextIndex += 1;

      const resolved = await resolveCompatibleStation(candidates[currentIndex], currentIndex);
      if (resolved && compatible.length < RESULT_LIMIT) {

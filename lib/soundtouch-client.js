...
const BOSE_STATION_WRAPPER_URL = "https://content.api.bose.io/core02/svc-bmx-adapter-orion/prod/orion/station";
...
function buildBoseStationLocation(streamUrl, { name = "Internet Radio", imageUrl = "" } = {}) {
  const data = {
    name: name || "Internet Radio",
    imageUrl: imageUrl || "",
    streamUrl,
  };
  const encodedData = Buffer.from(JSON.stringify(data)).toString("base64");
  return `${BOSE_STATION_WRAPPER_URL}?data=${encodeURIComponent(encodedData)}`;
}
...
async function storeNativePreset(host, preset, { name, streamUrl, imageUrl = "" }) {
  ...
  const presetName = name || `Preset ${presetNumber}`;
  const location = buildBoseStationLocation(streamUrl, {
    name: presetName,
    imageUrl,
  });
  const body = [
    `<preset id="${presetNumber}">`,
    `<ContentItem source="LOCAL_INTERNET_RADIO" type="stationurl" location="${escapeXml(location)}" sourceAccount="" isPresetable="true">`,
    `<itemName>${escapeXml(presetName)}</itemName>`,
    imageUrl ? `<containerArt>${escapeXml(imageUrl)}</containerArt>` : "<containerArt />",
    "</ContentItem>",
    "</preset>",
  ].join("");

  return requestSoundTouch(host, "/storePreset", body);
}
...
async function sendKey(host, key) {
  ...
  const requestKey = (state) => request({
    ...
    body: `<key state="${state}" sender="SoundTouchBridge">${escapedKey}</key>`,
  });
